...
async function submit() {
  ...
  try {
    ...
    if (err.response?.status === 404) {
      ...
      error.value = err.message?.includes('Network') ? 'Cannot reach server. Is the API running on port 5000?' : 'Login failed.'
    }
  } finally {
    loading.value = false
  }
...
<template>
  <div class="mx-auto max-w-sm space-y-8 pt-8">
    <div class="text-center">
      ...
      <p class="mt-1 text-sm text-slate-500">Access your notes</p>
    </div>
    ...
    <p class="text-center text-sm text-slate-500">
      ...
      <router-link to="/register" class="font-medium text-primary-600 hover:underline">Register</router-link>
    </p>
  </div>
</template>
